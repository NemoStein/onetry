...
package org.flixel.plugin.photonstorm.FX 
{
	...
	public class GlitchFX extends BaseFX
	{
		...
		private var fromCamera:Boolean;
		...
		public function createFromCamera(source:FlxCamera, maxGlitch:uint, maxSkip:uint):void
		{
			sprite = new FlxSprite(source.x, source.y).makeGraphic(source.width + maxGlitch, source.height, 0x0);
		...
		public function draw():void
		{
			if (ready)
			{
				...
				{
					//Registry.cam.buffer.copyPixels(canvas, new Rectangle(0,0,canvas.width,canvas.height), new Point());
				}
			}
		}
	}
