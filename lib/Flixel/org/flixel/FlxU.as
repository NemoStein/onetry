package org.flixel
{
	...
	public class FlxU
	{
		 ...
		 * @param	ShowMS		The number of.
		 * 
		 * @return	A nicely formatted <code>String</code>, like "1:03".
		 */
		static public function formatTime(Seconds:Number, ShowMS:Boolean = false, ToFixed:Number = 2):String
		{
			var timeString:String = int(Seconds / 60) > 10 ? int(Seconds / 60).toString() : "0" + int(Seconds / 60) + ":";
			...
			{
				timeString += ".";
				timeStringHelper = (Seconds - int(Seconds)) * Math.pow(10, ToFixed);
				
				if (timeStringHelper != 0)
				{
					while (--ToFixed > 0)
					{
						if (timeStringHelper < Math.pow(10, ToFixed))
						{
							timeString += "0";
						}
					}
